	...
	?>
		<div class="swl-widgetField -media <?=esc_attr( $field_class )?>">
			...
			<div class="media_btns">
				<input class="button" type="button" name="media-upload-btn" data-id="<?=esc_attr( $id )?>" value="画像を選択" />
				<input class="button" type="button" name="media-clear" value="画像を削除" data-id="<?=esc_attr( $id )?>" />
			</div>
		</div>
		<?php
	}
