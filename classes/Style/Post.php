...
class Post {
	...
	public static function btn() {
		...
		$btn_blue2  = $EDITOR['color_btn_blue2'];
		$btn_green  = $EDITOR['color_btn_green'];
		$btn_green2 = $EDITOR['color_btn_green2'];

		Style::add_root( '--color_btn_red', $btn_red );
		Style::add_root( '--color_btn_blue', $btn_blue );
		Style::add_root( '--color_btn_green', $btn_green );
		Style::add_root( '--color_btn_blue_dark', \SWELL_Theme::get_rgba( $btn_blue, 1, -.25 ) );
		Style::add_root( '--color_btn_red_dark', \SWELL_Theme::get_rgba( $btn_red, 1, -.25 ) );
		Style::add_root( '--color_btn_green_dark', \SWELL_Theme::get_rgba( $btn_green, 1, -.25 ) );

		// グラデーション設定
		$is_btn_gradation = $EDITOR['is_btn_gradation'];
		if ( is_admin() || $is_btn_gradation ) {
			Style::add_root( '--color_btn_red2', $btn_red2 );
			Style::add_root( '--color_btn_blue2', $btn_blue2 );
			Style::add_root( '--color_btn_green2', $btn_green2 );
		}
		if ( $is_btn_gradation ) {
			Style::add(
				['.is-style-btn_normal.red_ a', '.is-style-btn_shiny.red_ a' ],
				'background: linear-gradient(100deg,var(--color_btn_red) 0%,var(--color_btn_red2) 100%)'
			);
			Style::add(
				['.is-style-btn_normal.blue_ a', '.is-style-btn_shiny.blue_ a' ],
				'background: linear-gradient(100deg,var(--color_btn_blue) 0%,var(--color_btn_blue2) 100%)'
			);
			Style::add(
				['.is-style-btn_normal.green_ a', '.is-style-btn_shiny.green_ a' ],
				'background: linear-gradient(100deg,var(--color_btn_green) 0%,var(--color_btn_green2) 100%)'
			);
		}

		// ボタンの丸み
		Style::add( '.is-style-btn_normal a', 'border-radius:' . $EDITOR['btn_radius_normal'] );
		Style::add( '.is-style-btn_solid a', 'border-radius:' . $EDITOR['btn_radius_solid'] );
		Style::add( '.is-style-btn_shiny a', 'border-radius:' . $EDITOR['btn_radius_shiny'] );
		Style::add( '.is-style-btn_line a', 'border-radius:' . $EDITOR['btn_radius_line'] );
	...
	public static function balloon() {
		...
		$colors = [
			'gray' => [
				'bg'     => $EDITOR['color_bln_gray_bg'],
				'border' => $EDITOR['color_bln_gray_border'],
			],
			'green' => [
				'bg'     => $EDITOR['color_bln_green_bg'],
				'border' => $EDITOR['color_bln_green_border'],
			],
			'blue' => [
				'bg'     => $EDITOR['color_bln_blue_bg'],
				'border' => $EDITOR['color_bln_blue_border'],
			],
			'red' => [
				'bg'     => $EDITOR['color_bln_red_bg'],
				'border' => $EDITOR['color_bln_red_border'],
			],
			'yellow' => [
				'bg'     => $EDITOR['color_bln_yellow_bg'],
				'border' => $EDITOR['color_bln_yellow_border'],
			],
		];
	...
	public static function iconbox( $icon_box_type, $icon_bigbox_type ) {
		...
		// Big
		$big_box_css = ['position:relative', 'padding:2em 1.5em 1.5em' ];
	...
	public static function h2( $h2_type, $color_htag ) {
		...
		switch ( $h2_type ) {
			case 'b_left':
				...
				break;
			case 'b_left2':
				$colH_thin    = \SWELL_Theme::get_rgba( $color_htag, 0.15 );
			...
			case 'tag_normal':
				...
				$h2[] = 'background:' . \SWELL_Theme::get_rgba( $color_htag, 0.03 );
				break;
			case 'tag':
				$col_thin = \SWELL_Theme::get_rgba( $color_htag, 0.05, 0.25 );
			...
			case 'stitch':
				...
				if ( $h2_type === 'stitch' ) {
					...
					$h2[]        .= 'background:' . \SWELL_Theme::get_rgba( $color_htag, 0.05, 0.25 );
					$h2__before[] = 'border:dashed 1px ' . \SWELL_Theme::get_rgba( $color_htag, 0.5 );
				}
				break;
			case 'letter':
	...
	public static function h3( $h3_type, $color_htag ) {
		...
		switch ( $h3_type ) {
			...
			case 'main_gray':
			case 'main_thin':
				$col = ( $h3_type === 'main_gray' ) ? 'rgba(150,150,150,.2)' : \SWELL_Theme::get_rgba( $color_htag, 0.2, 0.25 );
				...
				break;
			case 'gradation':
				$col          = \SWELL_Theme::get_rgba( $color_htag, 0.2, 0.5 );
				$h3[]         = 'padding:0 .5em .5em';
				$h3__before[] = 'content:"";width:100%;height:2px';
