...
<header id="header" class="l-header <?=esc_attr( $header_class )?>" data-spfix="<?=$SETTING['fix_header_sp'] ? '1' : '0'?>">
	<?php SWELL_FUNC::get_parts( 'parts/header/head_bar' ); // ヘッダーバー ?>
	<div class="l-header__inner l-container">
		<div class="l-header__logo">
			<?php echo SWELL_PARTS::head_logo( $SETTING['header_transparent'] ); ?>
		...
		<?php endif; ?>
		<?php SWELL_FUNC::get_parts( 'parts/header/sp_btns' ); // メニューボタン & カスタムボタン ?>
	</div>
	<?php
	if ( has_nav_menu( 'sp_head_menu' ) ) : // SP用ヘッダーメニュー
		SWELL_FUNC::get_parts( 'parts/header/sp_head_nav', $SETTING['sp_head_nav_loop'] );
	endif;
	?>
</header>
<?php
...
// FIXヘッダー
if ( $SETTING['fix_header'] ) SWELL_FUNC::get_parts( 'parts/header/fix_header', $header_class );

// お知らせバー（下部表示）
if ( $SETTING['info_bar_pos'] === 'head_bottom' ) SWELL_FUNC::get_parts( 'parts/header/info_bar' );
