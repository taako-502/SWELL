...
<?php
	...
	?>
		...
		<?php
			$term_ppp = get_option( 'posts_per_page' ) + 1;
			$q_args   = [
				'post_type'           => 'post',
				'post_status'         => 'publish', // 非公開時期の投稿がキャッシュされないように
				'no_found_rows'       => true,
				'ignore_sticky_posts' => 1,
				'posts_per_page'      => $term_ppp,
				'tax_query'           => [
					[
						'taxonomy'         => $t['tax'],
						'field'            => 'id',
			...
			if ( isset( $cat_url ) && ( $term_ppp === $the_query_count ) ) :
				$page2url = false === strpos( $cat_url, '/?' ) ? '/page/2/' : '&paged=2';
			?>
				<div class="c-pagination c-tabBody__pager">
					<a href="<?=esc_url( $cat_url . $page2url )?>" class="page-numbers">
