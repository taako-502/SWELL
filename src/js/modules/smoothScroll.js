...
export function addSmoothScrollEvent(dom) {
	...
	for (let i = 0; i < linkElems.length; i++) {
		const theLink = linkElems[i];
		const targetVal = theLink.getAttribute('target');
		if ('_blank' === targetVal) continue;
		theLink.addEventListener('click', function (e) {
			const href = theLink.getAttribute('href');
			const splitHref = href.split('#');
			const targetID = splitHref[1];
			const targetElem = document.getElementById(targetID); // リンク先の要素（ターゲット）取得

			if (targetElem) {
				// e.preventDefault();
				smoothScroll(targetElem, smoothOffset);

				// スマホメニューが開いていれば閉じるように
